...
**Card art assets** (`static/images/`) are tracked, including all 78 card illustrations and the Lore artwork. Keep this directory included in Git so Render receives the images. The resolver prefers `.webp`, then `.jpg`, `.jpeg`, and `.png`, with the tracked portrait SVG as fallback. Restart the app after adding artwork to clear the cached path results.
